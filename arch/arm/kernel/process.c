...
void __print_symbol_fih(char *buffer_panic, int buffer_size, unsigned long address)
{
        int strlen_char = 0;

        memset(buffer_panic, 0, buffer_size);
        sprint_symbol(buffer_panic, address);
        strlen_char = strlen(buffer_panic);
        strlcpy((buffer_panic + strlen_char), "\n", buffer_size);
}

void print_symbol_fih(unsigned long addr, char *buffer_panic, int buffer_size)
{
        __print_symbol_fih( buffer_panic, buffer_size, (unsigned long) __builtin_extract_return_addr((void *)addr));
}
...
static void fih_write_panic_into_buffer(struct pt_regs *regs)
{
        ...
        int panic_string_len = PANIC_RAM_DATA_SIZE - sizeof(struct fih_panic_ram_data);
